...
*   [Ruby 基础课](https://www.python-oa.org/books/2021/ruby-beginner/)
...
打开命令行：在 macOS 中打开 Terminal.app，在 Windows 中要在开始菜单中选择“运行”，然后输入“cmd.exe”，建议使用cmder，一个第三方的Windows命令行工具，比cmd.exe体验好很多。本文中所有以 $ 开头的代码，都应该在命令行中执行。首先确认是否安装了 Ruby 的最新版本：
...
在 Windows 中，mysql的安装也十分简单。本教程选择使用 mysql-5.6.51-winx64 版。通过以下链接下载

```bash
https://downloads.mysql.com/archives/community/
```

下载完以后，解压下载的压缩包。并把bin所在的文件目录。例如

```bash
C:\mysql-5.6.51-winx64\bin
```

添加到系统环境变量中。然后就可以在cmder中查看MySQL版本了。

```sh
λ mysqld --version
mysqld  Ver 5.6.51 for Win64 on x86_64 (MySQL Community Server (GPL))
```
...
如果执行结果类似 `Rails 6.1.3`，那么就可以继续往下读了。
...
这个命令会在文件夹 `blog` 中创建名为 Blog 的 Rails 应用，并且使用mysql数据库引擎，rails new 执行完以后，会自动执行 `bundle install` 命令安装 Gemfile 中列出的 gem 及其依赖。
...
要访问应用，需要在开发设备中启动数据库服务。这里我们使用的是mysql数据库。在命令行中执行下面的指令

```bash
mysqld
```

TIP: 这个窗口执行后不要关闭，再开启一个窗口用来执行后续指令。

### 启动 Web 服务器

在启动Web服务器之前，我们需要先创建一下项目对应的数据库。请在 `blog` 文件夹中执行下面的命令：
...
TIP: 编译 CoffeeScript 和压缩 JavaScript 静态资源文件需要 JavaScript 运行时，如果没有运行时，在压缩静态资源文件时会报错，提示没有 `execjs`。安装Nodejs是一种快速安装JavaScript 运行时的办法。
